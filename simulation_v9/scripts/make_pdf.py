...
MD=os.path.join(HERE,"report_sim_v9.md");FIG=os.path.join(HERE,"figures")
...
for ln in open(MD,encoding="utf-8"):
    ...
    if s.startswith("# "):pdf.ln(2);pdf.set_font("Droid",size=15);pdf.multi_cell(W,8,s[2:]);pdf.set_font("Droid",size=9)
        ...
        c=re.sub(r"\*\*(.+?)\*\*",r"\1",s);c=re.sub(r"`([^`]+)`",r"\1",c);pdf.multi_cell(W,5,c)
pdf.add_page();pdf.set_font("Droid",size=14);pdf.cell(0,10,"附录：图表",ln=1);pdf.ln(2);pdf.set_font("Droid",size=9)
for fig in sorted(glob.glob(os.path.join(FIG,"*.png"))):
    ...
    pdf.ln(3)
out=os.path.join(HERE,"report_sim_v9.pdf");pdf.output(out);print("PDF:",out,os.path.getsize(out))
